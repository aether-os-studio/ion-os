...
QEMUFLAGS := -m 2G -smp 4 -serial stdio -d cpu_reset

override IMAGE_NAME := ion-$(ARCH)
...
run-hdd-x86_64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).img
	qemu-system-$(ARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=ovmf/ovmf-code-$(ARCH).fd,readonly=on \
		-hda $(IMAGE_NAME).img \
...
run-hdd-aarch64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).img
	qemu-system-$(ARCH) \
		-M virt \
		-cpu cortex-a72 \
		-device ramfb \
...
run-hdd-riscv64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).img
	qemu-system-$(ARCH) \
		-M virt \
		-cpu rv64 \
		-device ramfb \
...
run-hdd-loongarch64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).img
	qemu-system-$(ARCH) \
		-M virt \
		-cpu la464 \
		-device ramfb \
...
ifeq ($(ARCH),x86_64)
	PATH=$$PATH:/usr/sbin:/sbin sgdisk $(IMAGE_NAME).img -n 1:2048 -t 1:ef00 -m 1
	./limine/limine bios-install $(IMAGE_NAME).img
else
	PATH=$$PATH:/usr/sbin:/sbin sgdisk $(IMAGE_NAME).img -n 1:2048 -t 1:ef00
endif
...
ifeq ($(ARCH),x86_64)
	mcopy -i $(IMAGE_NAME).img@@1M limine/limine-bios.sys ::/boot/limine
	mcopy -i $(IMAGE_NAME).img@@1M limine/BOOTX64.EFI ::/EFI/BOOT
	mcopy -i $(IMAGE_NAME).img@@1M limine/BOOTIA32.EFI ::/EFI/BOOT
endif
ifeq ($(ARCH),aarch64)
	mcopy -i $(IMAGE_NAME).img@@1M limine/BOOTAA64.EFI ::/EFI/BOOT
endif
ifeq ($(ARCH),riscv64)
	mcopy -i $(IMAGE_NAME).img@@1M limine/BOOTRISCV64.EFI ::/EFI/BOOT
endif
ifeq ($(ARCH),loongarch64)
	mcopy -i $(IMAGE_NAME).img@@1M limine/BOOTLOONGARCH64.EFI ::/EFI/BOOT
endif
...
distclean:
	...
	rm -rf iso_root *.iso *.img kernel-deps limine ovmf
