#include <libs/klibc.hpp>
#include <libs/bitmap.hpp>

void bitmap::bitmap::init(std::uint8_t *data, std::size_t size)
{
    this->data = data;
    this->size = size;
    memset(data, 0, size / sizeof(*data) / 8);
}

bitmap::bitmap::bitmap()
{
}

bitmap::bitmap::~bitmap()
{
}

bool bitmap::bitmap::get(std::size_t index)
{
    std::size_t byte_index = index / 8;
    std::size_t bit_index = index % 8;

    return (data[byte_index] & (1 << bit_index)) != 0;
}

void bitmap::bitmap::set(std::size_t index, bool value)
{
    std::size_t byte_index = index / 8;
    std::size_t bit_index = index % 8;

    if (value)
    {
        data[byte_index] |= (1 << bit_index);
    }
    else
    {
        data[byte_index] &= ~(1 << bit_index);
    }
}

std::size_t bitmap::bitmap::find_range(bool value, std::size_t length)
{
    size_t count = 0, start_index = 0;
    uint8_t byte_match = value ? (uint8_t)-1 : 0;

    for (size_t byte_idx = 0; byte_idx < this->size / 8; byte_idx++)
    {
        size_t byte = this->data[byte_idx];

        if (byte == !byte_match)
        {
            count = 0;
        }
        else if (byte == byte_match)
        {
            if (length < 8)
            {
                return byte_idx * 8;
            }
            if (count == 0)
            {
                start_index = byte_idx * 8;
            }
            count += 8;
            if (count >= length)
            {
                return start_index;
            }
        }
        else
        {
            for (size_t bit = 0; bit < 8; bit++)
            {
                bool bit_value = (byte >> bit) & 1;
                if (bit_value == value)
                {
                    if (count == 0)
                    {
                        start_index = byte_idx * 8 + bit;
                    }
                    count++;
                    if (count == length)
                    {
                        return start_index;
                    }
                }
                else
                {
                    count = 0;
                }
            }
        }
    }

    return static_cast<std::size_t>(-1);
}

void bitmap::bitmap::set_range(std::size_t start, std::size_t end, bool value)
{
    if (start >= end || start >= this->size)
    {
        return;
    }

    size_t start_word = (start + 7) / 8;
    size_t end_word = end / 8;

    for (size_t i = start; i < start_word * 8 && i < end; i++)
    {
        this->set(i, value);
    }

    if (start_word > end_word)
    {
        return;
    }

    if (start_word <= end_word)
    {
        uint8_t fill_value = value ? (uint8_t)-1 : 0;
        for (size_t i = start_word; i < end_word; i++)
        {
            this->data[i] = fill_value;
        }
    }

    for (size_t i = end_word * 8; i < end; i++)
    {
        this->set(i, value);
    }
}
